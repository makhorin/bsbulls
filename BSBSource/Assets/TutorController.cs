    ...
    void Update ()
    {
        if (TextField.IsActive())
        {
            if (Input.GetKeyDown(_key.Value))
                _times--;
            var prt = Settings.MaxSpeed / Settings.DefaultTutorShowTime;
            var spd = Mathf.FloorToInt(Settings.CurrentSpeed / prt);

            if (Time.time - _showTime >= Settings.DefaultTutorShowTime - spd || _times <=0)
            {
